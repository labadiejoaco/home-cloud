...
export async function load({ fetch, params }) {
	try {
		const res = await fetch(`http://localhost:8080/api/${params.path}`);
...
export const actions = {
	create: async (event) => {
		...
		try {
			const res = await fetch(
				`http://localhost:8080/api/directory${path.startsWith('/') ? path : ''}`,
				{
					method: 'POST',
					body: JSON.stringify({
						name
	...
	upload: async (event) => {
		...
		try {
			const res = await fetch(
				`http://localhost:8080/api/upload${path.startsWith('/') ? path : ''}`,
				{
					method: 'POST',
					body: form
				}
